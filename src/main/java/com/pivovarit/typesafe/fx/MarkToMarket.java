...
public final class MarkToMarket {
    ...
    public static <F extends ReifiedCurrency, T extends ReifiedCurrency> Money<T> derive(FxForwardRate<F, T> bookedRate, FxForwardRate<F, T> marketRate, Money<F> bookedAmount, DealtAction action) {
        return derive(bookedRate.rate(), marketRate.rate(), bookedAmount, action);
    }

    public static <F extends ReifiedCurrency, T extends ReifiedCurrency> Money<T> derive(FxRate<F, T> bookedRate, FxRate<F, T> marketRate, Money<F> bookedAmount, DealtAction action) {
